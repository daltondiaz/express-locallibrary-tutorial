...
exports.index = function(req, res, next) {
    async.parallel({
      ...
      res.render('index', {title: 'Local Library Home',author: '@daltondiaz', error: err, data: results});
    });
};
...
exports.book_create_post = function(req, res, next) {
    ...
    if(errors){
      // If has some problems so we need to re-render our book
      // get all authors and genres for for
      async.parallel({
        authors: function(callback){
          Author.find(callback);
        },
        ...
        for(i=0; i< results.genres.length; i++){
          if(book.genre.indexOf(results.genres[i]._id) > -1){
            // Current genre is select. Set 'checked' flag
            results.genres[i].checked = 'true'
          }
        }
        res.render('book_form',{title: 'Create Book', authors: results.authors,
         genres: results.genres, errors: errors});
      });
...
exports.book_delete_get = function(req, res, next) {
    async.parallel({
      ...
      if(err){ return next(err); }
      res.render('book_delete',{title: 'Book Delete',
        book: results.book,
